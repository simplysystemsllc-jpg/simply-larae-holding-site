 ...
 *
 * Sends email notifications via Resend when RESEND_API_KEY is set.
 * - Admin notification to owner (with CC)
...
var FROM_EMAIL  = process.env.FROM_EMAIL  || "Simply LaRae <hello@simplyintegratedco.com>";
var BRAND_INQUIRY_SUBJECT = "Brand Partnership Inquiry";
...
module.exports = async function handler(req, res) {
  ...
  if (RESEND_API_KEY) {
    var isBrandInquiry = subject.indexOf(BRAND_INQUIRY_SUBJECT) !== -1;

    try {
      var adminEmailRes = await fetch("https://api.resend.com/emails", {
        method: "POST",
        headers: {
      ...
      if (!adminEmailRes.ok) {
        console.error("[RESEND ADMIN ERROR]", adminEmailRes.status, await adminEmailRes.text());
      } else {
    ...
    try {
      ...
      if (isBrandInquiry) {
        ...
        var contactName = dashIdx > -1 ? name.slice(0, dashIdx) : name;
        var companyName = dashIdx > -1 ? name.slice(dashIdx + 3) : "your company";
        confirmSubject = "Simply LaRae \u2014 Brand Partnership Inquiry Received";
        confirmHtml = buildConfirmationHtml(contactName,
          "<p>Thank you for your interest in partnering with Simply LaRae. We've received your inquiry on behalf of <strong>" + escape(companyName) + "</strong>.</p>" +
        ...
        confirmHtml = buildConfirmationHtml(name,
      ...
      var confirmRes = await fetch("https://api.resend.com/emails", {
        method: "POST",
        headers: {
          "Authorization": "Bearer " + RESEND_API_KEY,
          "Content-Type": "application/json",
...
function buildConfirmationHtml(name, bodyContent) {
  ...
  return "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><style>" +
    ...
    ".wrap{max-width:600px;margin:0 auto;background:#fff;border-radius:24px;overflow:hidden;border:1px solid #E8D5CF}" +
    ".hdr{background:#EED4CF;padding:40px;text-align:center}" +
    ".hdr h1{font-size:28px;font-weight:300;letter-spacing:.15em;color:#6E544E;margin:0}" +
    ...
    ".ftr{padding:24px 40px;border-top:1px solid #E8D5CF}" +
    ".ftr p{font-size:10px;color:#B09999;line-height:1.6}" +
    "</style></head><body>" +
    "<div class=\"wrap\">" +
